...
public class MeshDestroy : MonoBehaviour
{
    ...
    public void BreakMesh(Vector3 direction,float ExplodeForce)
    {
        ...
        for (var c = 0; c < CutCascades; c++)
        {
            for (var i = 0; i < parts.Count; i++)
            {
                ...
                var plane = new Plane(UnityEngine.Random.onUnitSphere, direction);
        ...
        for (var i = 0; i < parts.Count; i++)
        {
            ...
            parts[i].GameObject.GetComponent<Rigidbody>().AddForce(direction * ExplodeForce, ForceMode.Impulse);
    ...
    public class PartMesh
    {
        ...
        public void MakeGameobject(MeshDestroy original)
        {
            ...
            if (original.CutCascades / 2 > 1)
            {
               // var meshDestroy = GameObject.AddComponent<MeshDestroy>();
                //meshDestroy.CutCascades = (int)Math.Floor(Convert.ToDouble(original.CutCascades / 2));
                //meshDestroy.ExplodeForce = original.ExplodeForce;
            }
            else
            {
                var meshFade = GameObject.AddComponent<FadeObject>();
                meshFade.StartFade();
            }
        }
    }
