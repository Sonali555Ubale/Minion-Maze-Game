...
namespace EvolveGames
{
    ...
    public class PlayerController : MonoBehaviour
    {
        ...
        [SerializeField] public HitParticle hitParticle;
        [SerializeField] public float ExplodeForce = 5;
        [SerializeField, Range(1, 10)] float walkingSpeed = 3.0f;
        [SerializeField, Range(1, 10)] float attackDistance = 2f;
        ...
        void Update()
        {
            ...
            if (Input.GetMouseButtonDown(0))
            {
                ...
                if (Physics.Raycast(cam.ScreenPointToRay(Input.mousePosition), out RayHitObject, attackDistance))
                {
                    string tag = RayHitObject.collider.tag;
                    if (tag == MaterialTypes.Wood.ToString())
                    {
                        MeshDestroy HitRef = null;
                        RayHitObject.transform.TryGetComponent<MeshDestroy>(out HitRef);
                        if (HitRef != null)
                        {
                            Items.ani.SetTrigger("Attack");
                            hitParticle.direction = cam.transform.forward;
                            hitParticle.force = ExplodeForce;
                            hitParticle.Hitobject = RayHitObject.collider.gameObject;
                        }
                    }
                    else if(tag == MaterialTypes.Metal.ToString())
                    {
                        Items.ani.SetTrigger("Attack");
                        hitParticle.direction = cam.transform.forward;
                        hitParticle.Hitobject = RayHitObject.collider.gameObject;
            ...
            if(WallDistance != Physics.Raycast(GetComponentInChildren<Camera>().transform.position, transform.TransformDirection(Vector3.forward), out ObjectCheck, HideDistance, LayerMaskInt) && CanHideDistanceWall && !isRunning)
            {
